...
def run():
    ...
    scheduler = GradualWarmupSchedulerV2(optimizer, multiplier=config.WARMUP_FACTOR, total_epoch=config.WARMUP_EPO, after_scheduler=scheduler_cosine)
    ...
    for fold in range(config.N_FOLDS):
        ...
        dataset_train = LeafDataset(df_this, df_this['label'], transform=transforms_train)
        dataset_valid = LeafDataset(df_valid, df_valid['label'], transform=transforms_valid)

        train_loader = DataLoader(dataset_train, batch_size=config.TRAIN_BS, num_workers=config.NUM_WORKERS, shuffle=True)
        valid_loader = DataLoader(dataset_valid, batch_size=config.VALID_BS, num_workers=config.NUM_WORKERS, shuffle=False)
        ...
        for epoch in range(1, config.N_EPOCHS+1):
            ...
            avg_train_loss = train_loop_fn(model, train_loader, optimizer, loss_func, device, epoch, scaler)     
